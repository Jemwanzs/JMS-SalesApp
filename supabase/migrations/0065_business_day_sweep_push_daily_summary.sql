-- ============================================================================
-- 0065_business_day_sweep_push_daily_summary.sql
--
-- Web Push (Feature 3)'s first real, working trigger: fold a new
-- `push_daily_summary` report_jobs row into the SAME two places
-- run_business_day_sweep() already inserts a `daily_business_day_report`
-- row (the main per-location close, and the cross-midnight closing
-- pass) -- reusing the existing outbox drain (app/api/cron/outbox/
-- route.ts) instead of a second pipeline. Payload carries only
-- business_day_id/location_id, matching daily_business_day_report's own
-- shape -- the send step re-reads business_days.aggregates at delivery
-- time rather than trusting a stale denormalized copy of gross sales.
--
-- Full redefinition, not a diff -- this repo's established convention
-- (never edit an already-applied migration). Rest of the body below is
-- byte-for-byte identical to 0056's version except for the two new
-- inserts.
-- ============================================================================
