-- ============================================================================
-- 0075_deleted_sale_reporting_exclusions.sql
--
-- Two SQL-side aggregate functions have their own hardcoded
-- `status <> 'voided'` filter that -- a pre-existing gap, confirmed by
-- reading both, unrelated to this migration's own feature but directly
-- load-bearing for it -- doesn't even exclude 'corrected' today (migration
-- 0026 fixed that gap at the 8 TypeScript call sites but never touched
-- these two SQL functions). A newly 'deleted' sale (migrations 0073/0074)
-- must not survive into the automatic business-day close/relock aggregate
-- any more than a corrected one should, so both gaps are closed together
-- here. Full create-or-replace of each -- never edit an already-applied
-- migration; both bodies are otherwise byte-for-byte identical to their
-- current (0065 / 0019) versions.
-- ============================================================================
...
begin
  ...
  into v_gross, v_count
  from public.sales
  where business_day_id = p_business_day_id and status not in ('voided', 'corrected', 'deleted');
