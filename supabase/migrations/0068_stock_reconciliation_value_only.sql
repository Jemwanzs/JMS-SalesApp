-- ============================================================================
-- 0068_stock_reconciliation_value_only.sql
--
-- Fixes a real gap in migration 0067: stock_reconciliations.actual_quantity
-- has been NOT NULL since 0036, but a value-based product's reconciliation
-- (per the user's own spec) never does a physical UNIT count -- the
-- "Actual" side is an estimated Actual Remaining Value, not a quantity.
-- Widens the column to nullable and updates the generated variance/check
-- constraint to tolerate that, then redefines record_stock_reconciliation()
-- so p_actual_quantity is optional and every quantity-side computation
-- degrades gracefully to null rather than raising or silently coercing to
-- zero (a value-based reconciliation's row simply carries no quantity-side
-- variance at all, only the value-side one from 0067).
-- ============================================================================

alter table public.stock_reconciliations alter column actual_quantity drop not null;
...
begin
  ...
  from pg_constraint
  where conrelid = 'public.stock_reconciliations'::regclass
    and contype = 'c'
    and pg_get_constraintdef(oid) ilike '%variance_reason%';

  if v_conname is not null then
    execute format('alter table public.stock_reconciliations drop constraint %I', v_conname);
  end if;
end $$;

alter table public.stock_reconciliations add constraint stock_reconciliations_variance_reason_check
  check (
    actual_quantity is null
    or variance_reason is not null
    or actual_quantity = opening_quantity + stock_in_quantity - stock_out_quantity
  );
...
begin
  ...
  if v_product.stock_control_method = 'quantity' and p_actual_quantity is null then
    raise exception 'An actual physical count is required for a quantity-controlled product';
  end if;
