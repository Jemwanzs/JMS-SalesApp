...
export async function setNotesFieldEnabledAction(
  tenantId: string,
  tenantSlug: string,
  enabled: boolean
  ...
  try {
    await new TenantService(supabase).setSetting(tenantId, "notes_field_enabled", enabled, user.id);
    ...
    await new AuditService(createServiceRoleClient())
      .log({
        ...
        entityId: "notes_field_enabled",
        newValues: { notes_field_enabled: enabled },
      })
      .catch(() => {});
