...
export async function setProductRankingEnabledAction(
  tenantId: string,
  tenantSlug: string,
  enabled: boolean
  ...
  try {
    await new TenantService(supabase).setSetting(tenantId, "product_ranking_enabled", enabled, user.id);
    ...
    await new AuditService(createServiceRoleClient())
      .log({
        ...
        entityId: "product_ranking_enabled",
        newValues: { product_ranking_enabled: enabled },
      })
      .catch(() => {});
