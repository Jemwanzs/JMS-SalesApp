...
export async function setHistoryEnabledAction(
  tenantId: string,
  tenantSlug: string,
  enabled: boolean
  ...
  try {
    await new TenantService(supabase).setSetting(tenantId, "history_enabled", enabled, user.id);
    ...
    await new AuditService(createServiceRoleClient())
      .log({
        ...
        entityId: "history_enabled",
        newValues: { history_enabled: enabled },
      })
      .catch(() => {});
