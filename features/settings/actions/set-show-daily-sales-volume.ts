...
export async function setShowDailySalesVolumeAction(
  tenantId: string,
  tenantSlug: string,
  enabled: boolean
  ...
  try {
    await new TenantService(supabase).setSetting(tenantId, "show_daily_sales_volume", enabled, user.id);
    ...
    await new AuditService(createServiceRoleClient())
      .log({
        ...
        entityId: "show_daily_sales_volume",
        newValues: { show_daily_sales_volume: enabled },
      })
      .catch(() => {});
