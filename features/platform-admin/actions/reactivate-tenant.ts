...
export async function reactivateTenantAction(
  ...
  try {
    ...
    await new PlatformAdminService(createServiceRoleClient()).reactivateTenant(platformAdminId, tenantId, reason);
  } catch (err) {
    return { error: err instanceof Error ? err.message : "Could not reactivate tenant" };
  }
