...
**Sale Date correction is the one exception to the trigger-only design above.** Migration `0078` applies a date-only correction **in place** on the existing sale row (no `status` transition, no replacement row — see `08-sales-engine.md`'s "Correcting a sale's date"), so neither trigger fires. Instead, `_apply_sale_date_change()` directly updates that sale's own existing `stock_movements` row's `occurred_on` to the new date (same `reference_id`, same movement — genuinely relocated, not reversed-and-rededucted). A combined correction (date change alongside an amount/quantity/product change) still gets the normal trigger-driven reversal/re-deduction for the non-date fields, since that part still goes through the replacement-row path; the date-only step then applies to whichever row is now "the sale."
