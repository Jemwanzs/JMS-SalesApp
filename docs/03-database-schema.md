...
| `sales` | `id`, `tenant_id`, `location_id`, `business_day_id`, `product_id`, `sale_number`, `barcode_reference`, `product_name_snapshot`, `product_image_snapshot`, `expected_price_snapshot`, `actual_amount`, `quantity` (nullable), `recorded_by`, `sale_date`, `sale_time`, `status` (open/locked/corrected/voided/reversed/deleted), `idempotency_key` — unique(`tenant_id`,`idempotency_key`) | Tenant — highest index load in the schema |
...
| `products` (additive columns) | `tracks_inventory` (boolean, default false), `unit_of_measure` (nullable text), `unit_of_measure_is_custom` (boolean), `low_stock_threshold` (nullable numeric), `cost_price` (nullable numeric(12,2), migration `0067` — powers value-based stock control) | Tenant |
| `stock_movements` | `id`, `tenant_id`, `location_id` (nullable), `product_id`, `product_name_snapshot`, `unit_of_measure_snapshot`, `movement_type` (opening_stock/stock_in/stock_out/adjustment_increase/adjustment_decrease/damaged/expired/lost/reconciliation_variance/sale/sale_reversal — `sale`/`sale_reversal` added by migration `0067` for tenant-wide stock-control-method integration), `quantity` (signed — positive increases the balance, negative decreases it), `reason` (nullable, required by a check constraint for every type except opening_stock/stock_in/stock_out/sale/sale_reversal), `reference_type` (manual/reconciliation/sale), `reference_id`, `recorded_by`, `occurred_on`, `unit_cost_snapshot`/`unit_price_snapshot` (nullable numeric, migration `0067` — value-based reconciliation) | Tenant — immutable append-only ledger, no UPDATE/DELETE policy, same pattern as `sales` |
...
| `stock_reconciliations` | `id`, `tenant_id`, `location_id` (nullable), `product_id`, `reconciliation_date`, `opening_quantity`, `stock_in_quantity`, `stock_out_quantity`, `expected_closing_quantity` (generated), `actual_quantity` (nullable since migration `0068` — value-controlled tenants reconcile by value instead), `variance` (generated), `variance_reason` (nullable, required by a check constraint whenever `variance <> 0`), `recorded_by`, plus value-based columns added by migrations `0067`/`0068` for value-controlled tenants: `opening_value`, `stock_added_value`, `expected_sales_value`, `actual_recorded_sales`, `actual_remaining_value`, `valid_adjustments_value`, `unexplained_variance_value`, `status` (balanced/within_tolerance/variance/material_variance) — one per product per day (coalesce-normalized unique index, location-independent since there's no per-location stock UI yet) | Tenant — no INSERT/UPDATE/DELETE policy for `authenticated`; writable only via `record_stock_reconciliation()` |
